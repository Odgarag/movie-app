...
import { getTopRatingApi } from '@/hooks/GetTopRatingApi'
...
export const TopRating = () => {
  const [topRating, setTopRating] = useState<UpComingMovie[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const nowPlaying = async () => {
      setLoading(true)
      const response = await getTopRatingApi()
      const firstTen = response?.results.splice(0, 10)
      setTopRating(firstTen)
      setLoading(false)
    }
    nowPlaying()
  }, [])
  ...
  return (
    <div className="relative top-80 container mx-auto mt-[52px] sm:top-100">
      <div className="flex justify-between mb-[36px] mx-[20px] sm:mx-[0px] sm:[32px]">
        <p className="flex text-2xl">Top Rating</p>
        <Link href={'./topRating'}>
